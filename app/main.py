...
# Static, frozen snapshot of one edition served at /sample/* with no Google
# login. Generated by scripts/build_sample_snapshot.py into app/sample_data/;
# the files are committed and only change when that script is re-run.
_SAMPLE_DIR = REPO_ROOT / "app" / "sample_data"


def _sample_file(name: str) -> str:
    path = _SAMPLE_DIR / name
    if not path.exists():
        raise HTTPException(404, "Sample data not built. Run scripts/build_sample_snapshot.py")
...
def _sample_json(name: str) -> dict:
    ...
    return _json.loads(_sample_file(name))


def _sample_meta() -> dict:
    try:
        return _sample_json("meta.json")
    except HTTPException:
        return {"date": "", "masthead_name": "Daily"}


def _sample_chrome(active: str) -> dict:
    """Chrome context for sample pages — mirrors _chrome_context but with
    auth flags off and the sample flag on so _nav.html renders /sample/*
    links and edit actions become no-op toasts."""
    meta = _sample_meta()
    return {
        "is_admin": False,
        "is_personalized": True,
        "masthead_name": meta.get("masthead_name") or "Daily",
        "active_nav": active,
        "sample": True,
        "edition_date": meta.get("date") or "",
    }


@app.get("/sample", response_class=HTMLResponse)
def sample_home(request: Request):
    return templates.TemplateResponse(
        request,
        "viewer.html",
        {
            "edition_html": _sample_file("home.html"),
            "prev_date": "",
            "next_date": "",
            "next_date_allowed": False,
            "today_str": "",
            "edition_generated_at": None,
...
def sample_pdf():
    ...
    if not path.exists():
        raise HTTPException(404, "Sample PDF not built.")
    return Response(
        path.read_bytes(),
        media_type="application/pdf",
        headers={"Content-Disposition": 'inline; filename="linh-times-sample.pdf"'},
    )


@app.get("/sample/movies", response_class=HTMLResponse)
def sample_movies(request: Request):
    data = _sample_json("movies.json")
    return templates.TemplateResponse(
        request,
        "admin_movies.html",
        {
            "all_ratings": _ALL_MOVIE_RATINGS,
            "default_ratings": data.get("default_ratings") or [],
            **_sample_chrome("movies"),
        },
    )


@app.get("/sample/movies/data")
def sample_movies_data():
    data = _sample_json("movies.json")
    return {"movies": data.get("movies") or [], "cache_age_seconds": 0}


@app.get("/sample/stocks", response_class=HTMLResponse)
def sample_stocks(request: Request):
    data = _sample_json("stocks.json")
    return templates.TemplateResponse(
        request,
        "admin_stocks.html",
        {"symbols": data.get("symbols") or [], **_sample_chrome("stocks")},
    )
...
def sample_data(request: Request):
    return templates.TemplateResponse(
        ...
        "data.html",
        {
            ...
            "settings_json": _sample_json("data.json"),
            **_sample_chrome("data"),
        },
    )
