...
def _render_headline_block(eligible_titles: list[str]) -> str:
    ...
    return (
        "## Headline (top-of-front-page lead article)\n\n"
        "In addition to the section content, return ONE `headline` object "
        "— a single front-page newspaper lead drawn from one of these topics:\n"
        f"{bullets}\n\n"
        "Rules:\n"
        "  - The body covers ONE story only. NO 'in other news', NO "
        "'meanwhile', NO 'besides this', NO 'elsewhere'. Stay on that "
        "one story.\n"
        "  - Write the body as a single flowing newspaper article. "
        "Plain prose only — NO section headings (no 'WHAT HAPPENED', "
        "'WHO IS INVOLVED', 'WHY IT MATTERS', etc.), NO bullet lists, "
